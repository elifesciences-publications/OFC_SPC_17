function [classify_output, classify_output2] = bo_misclass_wrapper_shuffle(neuron_index,...
    psth_AB_trials, psth_CD_trials, steps,...
    numperms, enssizes, max_A_trial, max_B_trial)
...
for enssize=enssizes(length(enssizes):-1:1)       
        for ii = numperms:-1:1
            ...
            [classify_output(ii,ee,:), classify_output2(ii,ee).blerg] = bo_psthmisclassify_constA_shuffle(thepsth1,thepsth2,steps);
        end
        ee = ee-1;
end