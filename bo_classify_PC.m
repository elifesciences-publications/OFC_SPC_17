...
filename(1) = {'PC1_wo25.mat'};
filename(2) = {'PC2_wo25.mat'};
...
for fi =1:2;
    ...
    for trials=6:-1:1;
        ...
        for nn = 1:size(site,1)
            ...
            %cue_meanBK_tr(nn,4*(trials-1)+1:4*(trials)) = squeeze(nanmean(meanpsth(nn,:,find(steps>-30,1,'first'):find(steps<-10,1,'last')),3));%-squeeze(mean(mean(yyy(nn,:,:,:),4),3));
            %cue_meanBK_tr(nn,4*(trials-1)+1:4*(trials)-2) = squeeze(nanmean(meanpsth(nn,3:4,find(steps>-30,1,'first'):find(steps<-0,1,'last')),3));%-squeeze(mean(mean(yyy(nn,:,:,:),4),3));
            %cue_meanBK_tr(nn,4*(trials-1)+1+2:4*(trials)) = squeeze(nanmean(meanpsth(nn,3:4,find(steps>-30,1,'first'):find(steps<-0,1,'last')),3));%-squeeze(mean(mean(yyy(nn,:,:,:),4),3));
            cue_meanBK_tr(nn,4*(trials-1)+1:4*(trials)) = [squeeze(nanmean(meanpsth(nn,3:4,find(steps>-30,1,'first'):find(steps<-0,1,'last')),3)),squeeze(nanmean(meanpsth(nn,3:4,find(steps>-30,1,'first'):find(steps<-10,1,'last')),3))];%-squeeze(mean(mean(yyy(nn,:,:,:),4),3));

        end
    end
    
   cue_mean_tr(isnan(cue_mean_tr))=0;
   cue_meanBK_tr(isnan(cue_meanBK_tr))=0;
    ...
    iindex=[];for ii = [3,1,4,2],iindex = [iindex,ii,ii+4,ii+8,ii+12,ii+16,ii+20];end,
    
    figure(1111);xxx=corr(zscore(cue_mean_tr')');subplot(1,2,fi),imagesc(xxx(iindex,iindex)),
    caxis([-.5,.5]),colorbar,colormap(flipud(pink)),
    box off,set(gca,'XTick',[3.5 9.5 15.5 21.5]),set(gca,'YTick',[3.5 9.5 15.5 21.5])
    set(gca,'XTickLabel',{'A','B','C','D'}),set(gca,'YTickLabel',{'A','B','C','D'})
       ...
       neuronvector=[size(site,1),10,25,50,60,65];
       % neuronvector=neuronvector(neuronvector<size(site,1));
        
        for numneurons=neuronvector(1)
            ncount = ncount+1;
            ecount=0;
            for ee = numsims:-1:1
                ...
                ensembleorder=randi(size(site,1),1,numneurons);
...
figure(1112),subplot(1,2,fi),
imagesc(100*squeeze(mean(classconf(1,1,fi,[3,1,4,2],[3,1,4,2],:),6))/6),
caxis([0 100]),colormap(flipud(pink)),
box off,set(gca,'XTick',[1 2 3 4]),set(gca,'YTick',[1 2 3 4])
set(gca,'XTickLabel',{'A','B','C','D'}),set(gca,'YTickLabel',{'A','B','C','D'})
...
pltct =0;
n_ens_sizes = size(classconf,1);
for ctrl_num=1:3
    for nn = n_ens_sizes:-1:1
        for ii = 2:-1:1,
            betweenpair(nn,ii,:)=nanmean([squeeze(classconf(nn,ctrl_num,ii,2,3,:)),squeeze(classconf(nn,ctrl_num,ii,3,2,:)),squeeze(classconf(nn,ctrl_num,ii,1,4,:)),squeeze(classconf(nn,ctrl_num,ii,4,1,:)),squeeze(classconf(nn,ctrl_num,ii,2,1,:)),squeeze(classconf(nn,ctrl_num,ii,1,2,:)),squeeze(classconf(nn,ctrl_num,ii,3,4,:)),squeeze(classconf(nn,ctrl_num,ii,4,3,:))],2)/24;
        end,
    end
    for nn = n_ens_sizes:-1:1
        for ii = 2:-1:1,
            withinpair(nn,ii,:)=nanmean([squeeze(classconf(nn,ctrl_num,ii,1,3,:)),squeeze(classconf(nn,ctrl_num,ii,3,1,:)),squeeze(classconf(nn,ctrl_num,ii,2,4,:)),squeeze(classconf(nn,ctrl_num,ii,4,2,:))],2)/24;
        end,
    end
    for nn = n_ens_sizes:-1:1
        for ii = 2:-1:1,
            samepair(nn,ii,:)=nanmean([squeeze(classconf(nn,ctrl_num,ii,1,1,:)),squeeze(classconf(nn,ctrl_num,ii,2,2,:)),squeeze(classconf(nn,ctrl_num,ii,3,3,:)),squeeze(classconf(nn,ctrl_num,ii,4,4,:))],2)/24;
        end
    end
...
%     end
    ...
    histvec=-1.013:.0125:1;
    pltct=pltct+2;
    
    for xx = n_ens_sizes:-1:1
        test_stat(xx,1,ctrl_num) = (1+sum((squeeze(betweenpair(xx,2,:))'-squeeze(withinpair(xx,2,:))')>=0))/(numsims+1);
        test_stat(xx,2,ctrl_num) = (1+sum((squeeze(withinpair(xx,1,:))'-squeeze(withinpair(xx,2,:))')>=0))/(numsims+1);
        
        figure(12341+xx);
        subplot(3,2,pltct-1),h=bar(histvec,histc(squeeze(withinpair(xx,1,:))'-squeeze(withinpair(xx,2,:))',histvec));shading flat,box off,xlim([-.2 .2]),ylim([0 370]),set(h,'barWidth',.8,'facecolor',[.2 .2 .2])
        hold on,text(.050,250,['p = ',num2str(test_stat(xx,2,ctrl_num))])
        hold on,text(0+.0250,400,'day 1 > day 2')
        hold on,text(-.187,400,'day 2 > day 1')
        
        plot ([0 0],[0 250],'r--')
        
        subplot(3,2,pltct),h=bar(histvec,histc(squeeze(betweenpair(xx,2,:))'-squeeze(withinpair(xx,2,:))',histvec));shading flat,box off,xlim([-.2 .2]),ylim([0 370]),set(h,'barWidth',.8,'facecolor',[.2 .2 .2])
        hold on,text(.050,250,['p = ',num2str(test_stat(xx,1,ctrl_num))])
        hold on,text(0+.0250,400,'unpaired cue >')
        hold on,text(-.187,400,'paired cue >')
        plot ([0 0],[0 250],'r--')
        
        
    end
    
    betweenpair2 = squeeze(betweenpair(1,:,:));
    withinpair2 = squeeze(withinpair(1,:,:));
    samepair2 = squeeze(samepair(1,:,:));
    
    figure(301)
    subplot(3,1,ctrl_num)
    bar_mean_vec = [nanmean(betweenpair2,2);nanmean(withinpair2,2);nanmean(samepair2,2)]/1;
    bar_std_vec = 2*([nanstd(betweenpair2,[],2);nanstd(withinpair2,[],2);nanstd(samepair2,[],2)]/1)/sqrt(1);

    errorbar(bar_mean_vec(:),bar_std_vec(:),'k','LineStyle','none'),hold on
    h=bar(bar_mean_vec(:));
    box off,set(gca,'XTick',[1 2 3 4 5 6])%,set(gca,'YTick',[1 2 3 4])
    set(gca,'XTickLabel',{'Between-pair (d1)','Between-pair (d2)','Within-pair (d1)','Within-pair (d2)','Same-cue (d1)','Same-cue (d2)'})%,set(gca,'YTickLabel',{'A','B','C','D'})
    shading flat,box off,set(h,'barWidth',.8,'facecolor',[.2 .2 .2],'LineStyle','none')
    ylim([0 .3])
    %xtickangle(45)


%[aa bb cc]=anova1([betweenpair2;withinpair2;samepair2]')
%[m.a m.b m.c]=multcompare(cc)
...
%     
%     for ii = 1:2,
%         withinpair(ii,:)=nansum([squeeze(classconfENS(ii,ctrl_num,1,3,:)),squeeze(classconfENS(ii,ctrl_num,3,1,:)),squeeze(classconfENS(ii,ctrl_num,2,4,:)),squeeze(classconfENS(ii,ctrl_num,4,2,:))],2);
%     end
%     for ii = 1:2,
%         samepair(ii,:)=nansum([squeeze(classconfENS(ii,ctrl_num,1,1,:)),squeeze(classconfENS(ii,ctrl_num,2,2,:)),squeeze(classconfENS(ii,ctrl_num,3,3,:)),squeeze(classconfENS(ii,ctrl_num,4,4,:))],2);
%     end
%     
%     %
%     % figure
...
%     clear betw* withinp* samep*
%     for ii = 1:2,
%         betweenpair(ii,:)=nanmean([squeeze(classconfNRN(ii,ctrl_num,2,3,:)),squeeze(classconfNRN(ii,ctrl_num,3,2,:)),squeeze(classconfNRN(ii,ctrl_num,1,4,:)),squeeze(classconfNRN(ii,ctrl_num,4,1,:)),squeeze(classconfNRN(ii,ctrl_num,2,1,:)),squeeze(classconfNRN(ii,ctrl_num,1,2,:)),squeeze(classconfNRN(ii,ctrl_num,3,4,:)),squeeze(classconfNRN(ii,ctrl_num,4,3,:))],2)/24;
%     end,
%     
%     for ii = 1:2,
%         withinpair(ii,:)=nanmean([squeeze(classconfNRN(ii,ctrl_num,1,3,:)),squeeze(classconfNRN(ii,ctrl_num,3,1,:)),squeeze(classconfNRN(ii,ctrl_num,2,4,:)),squeeze(classconfNRN(ii,ctrl_num,4,2,:))],2)/24;
%     end
%     for ii = 1:2,
%         samepair(ii,:)=nanmean([squeeze(classconfNRN(ii,ctrl_num,1,1,:)),squeeze(classconfNRN(ii,ctrl_num,2,2,:)),squeeze(classconfNRN(ii,ctrl_num,3,3,:)),squeeze(classconfNRN(ii,ctrl_num,4,4,:))],2)/24;
%     end
%     
%     betweenpair(betweenpair==0)=NaN;
%     withinpair(withinpair==0)=NaN;
...
% figure(311+ctrl_num)
% 
% bar_mean_vec = [nanmean(betweenpair,2);nanmean(withinpair,2);nanmean(samepair,2)]/1;
% bar_std_vec = ([nanstd(betweenpair,[],2);nanstd(withinpair,[],2);nanstd(samepair,[],2)]/1)/sqrt(100);
% 
% errorbar(bar_mean_vec(:),bar_std_vec(:)),hold on
% bar(bar_mean_vec(:))
% 
% % 
% % subplot(3,1,1)
...
%     [hval2(ctrl_num),pval2(ctrl_num)]=ttest(withinpair(1,:),withinpair(2,:));
%     
%     [aa bb cc]=anova1([betweenpair;withinpair;samepair]')
% [m.a m.b m.c]=multcompare(cc)
% 
% end
% 
