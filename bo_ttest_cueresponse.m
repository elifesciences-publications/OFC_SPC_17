...
for i = sum(indext):-1:1
    for j = 1:4
        if j<2.5
                [sign_p(i,j),sign_h(i,j)] = ranksum(nanmean(squeeze(psth_AB_ttest(i,j,1:6,cuetime)),2),squeeze(nanmean(psth_AB_ttestback(i,j,1:6,:),4)),'tail','right','alpha',0.05);
            

        else
                [sign_p(i,j),sign_h(i,j)] = ranksum(nanmean(squeeze(psth_CD_ttest(i,j,1:6,cuetime)),2),squeeze(nanmean(psth_CD_ttestback(i,j,1:6,:),4)),'tail','right','alpha',0.05);
